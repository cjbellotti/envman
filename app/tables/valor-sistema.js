...
function initialize (connectionData) {
	...
	this.read = function (query) {
		...
		query = generarWhere(query);
	 	...
	 	var response = request('GET', 'http://localhost:8080/envman-datalayer/datalayer/' + this.dc + '/DTVLA.DVM_VALOR_SISTEMA/' + query).body.toString('utf8');
	...
	this.lastId = function() {
		return request('GET', 'http://localhost:8080/envman-datalayer/datalayer/' + this.dc + '/DTVLA.DVM_VALOR_SISTEMA/lastid').ID;
	}
